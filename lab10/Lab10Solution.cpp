...
void displayReachableCitiesWithOneTicket(
                ...
                int visited[])       // Elements set to 1 when vertex is visited
{
    ...
    // Go through adjacency array for start City and display them
    for (Node *pTemp = graph[startCity]; pTemp!=NULL; pTemp=pTemp->pLink) {
        cout << pTemp->vertex << " ";
        
        // For each of the ones printed also mark it as being visited
        visited[ pTemp->vertex] = 1;
        
        // Also mark each child of these, for the two-ticket version
        for (Node *pTemp2 = graph[ pTemp->vertex]; pTemp2!=NULL; pTemp2=pTemp2->pLink) {
            visited[ pTemp2->vertex] = 1;
        }
    }//end for( Node *pTemp...
...
void displayReachableCitiesWithTwoTickets(
              int startCity,       // starting city
              Node *graph[],       // Adjacency lists representing the graph
              int visited[])       // Elements set to 1 when vertex is visited
{
